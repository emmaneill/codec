 ...
 */
#ifndef _BOE_TRADECAPTUREREPORTDECLINE_PACKET_H_
#define _BOE_TRADECAPTUREREPORTDECLINE_PACKET_H_
...
PACKED(class boeTradeCaptureReportDeclinePacket
{
public:
        uint64_t mTransactTime;
        char mTradeReportID[20];
        char mTradeCaptureDeclineTradeReportRefID[20];
        char mTradeID[8];
        uint32_t mTradeCaptureDeclineLastShares;
        uint64_t mLastPx;
        char mContraBroker[4];
        uint8_t mTradeCaptureDeclineReason;
        char mTradeCaptureDeclineText[60];
    ...
    boeTradeCaptureReportDeclinePacket ()
    {
        mTransactTime = 0;
        memset (mTradeReportID, 0, 20);
        memset (mTradeCaptureDeclineTradeReportRefID, 0, 20);
        memset (mTradeID, 0, 8);
        mTradeCaptureDeclineLastShares = 0;
        mLastPx = 0;
        memset (mContraBroker, 0, 4);
        mTradeCaptureDeclineReason = 0;
        memset (mTradeCaptureDeclineText, 0, 60);
    ...
    string getTradeCaptureDeclineTradeReportRefID ()
    {
        return getString (mTradeCaptureDeclineTradeReportRefID, sizeof (mTradeCaptureDeclineTradeReportRefID));
    }

    bool setTradeCaptureDeclineTradeReportRefID (const string& val)
    {
        return setString (mTradeCaptureDeclineTradeReportRefID, val, sizeof (mTradeCaptureDeclineTradeReportRefID));
    }
    
    string getTradeID ()
    {
        return getString (mTradeID, sizeof (mTradeID));
    }

    bool setTradeID (const string& val)
    {
        return setString (mTradeID, val, sizeof (mTradeID));
    }
    
    uint32_t getTradeCaptureDeclineLastShares ()
    {
        return mTradeCaptureDeclineLastShares;
    }

    bool setTradeCaptureDeclineLastShares (uint32_t val)
    {
        mTradeCaptureDeclineLastShares = val;
        return true;
    }
    ...
    bool setContraBroker (const string& val)
    {
        return setString (mContraBroker, val, sizeof (mContraBroker));
    }
    
    uint8_t getTradeCaptureDeclineReason ()
    {
        return mTradeCaptureDeclineReason;
    }

    bool setTradeCaptureDeclineReason (uint8_t val)
    {
        mTradeCaptureDeclineReason = val;
        return true;
    }
    
    string getTradeCaptureDeclineText ()
    {
        return getString (mTradeCaptureDeclineText, sizeof (mTradeCaptureDeclineText));
    }

    bool setTradeCaptureDeclineText (const string& val)
    {
        return setString (mTradeCaptureDeclineText, val, sizeof (mTradeCaptureDeclineText));
    }
